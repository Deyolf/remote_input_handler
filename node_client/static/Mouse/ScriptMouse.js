...
function move(x, y, speed, angle) {
  if (delayer % timedrop == 0) {
    speed = Math.floor(speed / 15);
    console.log(speed);
    console.log(x);
    console.log(y);
    let x_movement = Math.floor(speed * Math.cos(angle)) * multiplier;
    let y_movement = Math.floor(speed * Math.sin(angle)) * multiplier;
    console.log(`x_movement: ${x_movement}`);
    console.log(`y_movement: ${y_movement}`);
    socket.send(`${x_movement},${y_movement}`);
  }
  if (multiplier < 6) {
    if (multicount % buildup == 0) {
      multiplier++;
    }
    multicount++;
  }
